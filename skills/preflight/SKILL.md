...
1. **Read `flightdeck/rules.md`**. Resolve config per [protocol § Rule resolution order](protocol.md#rule-resolution-order): infer git from deck root `.git` (House Rule `this deck doesn't use git` overrides — when no-git, skip step 4's git note). Pre-3.0 keys, if present, are read but ignored.
...
4. **Passive git note (non-blocking — skip git entirely when no-git).** Gather `git branch --show-current` + `git status --short` in one pass; emit a one-line note only when a row below triggers, never a blocking "Resolve which?" prompt:
   ...
   Git-state notes say **"review before continuing"** — walkaround is the integrity-audit command; preflight reports git state only.
...
- **preflight 零写入** — 一切补偿/校验/version 写入归 landing/walkaround；preflight 只读报告。
