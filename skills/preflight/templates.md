...
- **`### Rules` is AI-authored, not a toggle catalog (3.0).** There is no magic-string vocabulary. When you state a persistent preference, the AI appends a free-prose rule here (with source/date) and honors it above the default. Settings not covered by frontmatter fields resolve via deck rules or built-in defaults:
  - `git` → **install precondition** (launch's doctor offers `git init`, refuses if declined or git absent); not configurable via `### Rules`.
  - `runtime` / `agents_md` → **recorded frontmatter fields** (stamped by launch / set at deck creation); not `### Rules` prose.
  - `agents_md` (`auto|off`, recorded field) → **landing reads the field, never probes for the file**: `auto` = `landing` regenerates `AGENTS.md` when a rendered cockpit field changed; `off` = `landing` never touches it. `/flightdeck:emit-agents-md` is the atomic bootstrap — it regenerates the file **and** flips the field to `auto` (a one-way intent record from a no-`AGENTS.md` start; already-`auto` → no-op). Editing the field (or re-running the emitter) is the only alignment lever — there is no re-probe-the-file bypass.
...
- **`<!-- flightdeck:project-specific -->` (shared/project boundary marker)** — a literal HTML-comment line inside a vendored file's body. **Above** it (frontmatter stripped) is the master-owned **shared region**; the marker and everything **below** it is the consumer-owned **project section**. The pull is **mechanical** — `--sync-pull` replaces the shared region with the master's body and keeps the marker + project section + frontmatter verbatim; staleness is a fingerprint over the normalized shared region (ignoring frontmatter, per-line trailing whitespace, trailing blank lines). No marker → the whole body is treated as the shared region; **vendorable master files ship the marker + a stub project section by default** so it travels to consumers, and a `synced` file that has drifted yet lacks the marker is flagged `marker-missing` (a pull would overwrite its whole body) rather than silently spliced.
...
current: Task 3 — wire the stage board-sync into landing SKILL.md
```

- `current:` names the next-to-execute (or in-progress) task. The board-sync advances it at each plan-task boundary (kept live mid-turn, sealed by **stage** at turn-end — see [exit-ritual § Stage](exit-ritual.md#stage--turn-end-persist--board-sync)); cockpit `## Next` quotes it.
...
- **`## Next` is the next concrete *single* action + a plan link** — AI-maintained, auto-written at stage (and on `idea→active` / a milestone). Keep it to one step; **progress checklists, rationale/citation lists, and milestone links belong in the plan's `## Progress`**, not here (they are not loaded every ritual). The user adjusts it by directing the AI, not by hand-editing.
- **`## Staged (awaiting land)` is an AUTO region** — derived (never hand-written), like `## In Progress`, from two frontmatter-decidable classes: `done`-not-archived workflow (specs/plans still in their source folder) + `stale`-with-`verify` knowledge (produced, pending review). The literal marker is `<!-- AUTO:staged -->` … `<!-- /AUTO -->` (`flightdeck_index.py` emits exactly this string). It is a **projection, not a truth source**: archiving a `done` artifact or flipping a `stale` one to `active` changes the truth source, and the item then drops out of `## Staged` on the next regen — no write to the view itself (same shape as a landed artifact leaving `## In Progress`). Empty body when nothing is staged.
- **`## Key Context` is the recovery slot — agent-judged, not AUTO.** The load-bearing literals a *next* session needs to resume: the file path under edit, a failing test name, an error string, a key function/value. Same maintenance model as `## Next` (AI-written at stage, not script-generated). Keep it to **literals, not prose**; nothing to carry → `- (none)`. Distinct from a knowledge artifact's body (that is durable knowledge; this is cockpit's transient resume hint) — both stress load-bearing literals, different layers.
  - **Per-entry drain (apply at stage).** An entry is **cleared** when the literal it points to no longer needs carrying forward: its target has been archived, has graduated into `docs/`, or the next session simply won't need it. An entry is **shrunk** when it has grown past a literal: a long entry collapses to a one-line pointer (link + hook); two same-source entries merge into one. The 80-line cap below is the ceiling, not the discipline — prune per-entry first.
- **`## Pending Review` is the sign-off queue — agent-judged, not AUTO.** Things the AI completed and self-judged done but you haven't eyeballed/approved yet (fits the AI-drives-autonomously, you-check-in-periodically loop). Each row: `- [<artifact/topic>] <what changed · how to look/verify> → after verification passes: <how to commit / next step>` — recording the post-verify action makes recovery **board-only** (next preflight knows what to do once you sign off, without the conversation). **Non-blocking** (you can land with items still queued — unlike `Hanging Tasks`) and **subjective** (human sign-off — unlike `verify:`, which is an objective run/test). It is also the home where stage surfaces a stale-knowledge `pending-review` note (a `stale` artifact awaiting your re-review), and it feeds the staged / landing banner's `[Pending]` field when non-empty ([protocol § Act-report-close loop](protocol.md#act-report-close-loop)). **Drain:** an item is removed when you sign off (at the manual land valve) once you confirm; nothing queued → `- (none)`.
- **Accumulator-drain principle.** Every non-AUTO section that *accumulates* (`Key Context`, `Pending Review`) MUST have an explicit drain condition, or it rots into a junk drawer. Draining is a judgment step, so it runs at **stage** (every turn — stage already rewrites cockpit by judgment); `Pending Review` sign-off itself is the manual land valve's action. `walkaround` only flags a suspected-stale / oversized `Key Context` as a non-blocking INFO — it never drains.
- **Length cap + role-creep check.** 80 lines is the hard ceiling — past it, trim immediately. Beyond line count, each field has a soft cap and a role (no changelog in `Updated`, no goal/criteria in `Focus`, no progress checklist in `## Next`); the authoritative per-field density + role-creep check runs at stage — see [exit-ritual § Length check](exit-ritual.md#cockpit-update--what-changes). `## In Progress` is AUTO and usually short; piled-up `active` is itself a focus-loss signal (walkaround INFO, never blocks).
