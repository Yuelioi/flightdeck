...
2. **Classify new knowledge** — heuristics (a)–(h), first-match wins; **no new knowledge is a valid outcome**; capture abandoned paths, not just shipped fixes; external review feedback folds into the reviewed spec's `## 评审纪要` (raw text stays in project-root `tmp/`); a **persistent behavior request** ("from now on…" / 以后每次) → append a free-prose rule to `rules.md` `### Rules` (source + date). [exit-ritual § Classification heuristics](../preflight/exit-ritual.md#classification-heuristics)
