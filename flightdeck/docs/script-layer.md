...
summary: 脚本层（flightdeck_index.py / flightdeck_lint.py）的职责——确定性 INDEX 再生、layout verdict、archivable_done、knowledge 嵌套 INDEX-of-INDEXes；以及 model/judgment 与 script/facts 的分工
---
...
| layout verdict（版本/结构落后与否） | 是否真要迁移、怎么改写关系边 |
| `archivable_done`（可归档集） | 状态该不该往前翻、要不要 land |
| INDEX↔folder drift 检测 | AGENTS.md 语义漂移、stray 文件可达性 |

这条线是脚本层全部设计的总纲：脚本吐一行 JSON / 一个 verdict 词，AI 据此判断。脚本在子进程跑、读文件不进上下文，模型只见结论——这是 token 经济性的来源。
...
- **specs/ 特殊分组**：idea 文件无日期前缀，混进 dated 的 active/done 会乱序，所以 AUTO 区**按 status 分组**（`### 待启动（idea）` / `### 进行中·完成（active·done）` / `### 已否决（scrapped）`）；`scrapped` 在 specs 的 root 计数里被跳过（要和可见行一致）。
...
- **layout verdict**（`--verdict`）：只读，吐 `current / compatible-behind / structural-behind / malformed` 之一。结构信号（残留 `charts/`/`landed/`/`sketches/`/`debriefs/` 目录、退役状态值、cockpit 缺 AUTO 区）→ `structural-behind`；版本数据**取自 `MIGRATION.md` frontmatter**（不是散文）。`--check` 报 drift 不写盘；`--archivable` 吐确定性可归档集。

## flightdeck_lint.py — 机械子集审计

`walkaround` 整套审计里，**无歧义、可机械判定**的那部分下沉到这里（status 合法性 / orphan plan / INDEX↔folder drift〔复用 `flightdeck_index.index_drift`〕/ dangling ref / stray file 子集），吐 `{"findings":[…]}` JSON，有 CRITICAL/WARNING 时 exit≠0。需要可达性整体推理、知识分类、迁移决策等**判断项**仍留模型——这正是上面那条分工线在 lint 侧的落点：lint 抓确定能抓的，剩下交给 walkaround 的 AI 判断。

## 版本守卫

`flightdeck_index.py` 编码了一个 layout 版本（`MIGRATION.md` 的 `current`）。对版本不同的 deck 跑再生有腐蚀风险，所以默认有版本守卫：deck 的 `rules.md` `version` 与脚本期望不符时**拒绝再生**（提示手工 fallback 或 `--force`）。`--verdict` / `--archivable` 是只读的，不受守卫限制。
