---
status: active
when_to_read: 改 flightdeck_conform（py/js）、合法字段集、conform 作用域/排除、脚本趟 vs AI 趟分工，或 /flightdeck:conform skill 之前
when_to_update: 改了 conform 的合法字段集、作用域/排除（如 references 是否参与）、脚本/AI 分工、rules 记录式配置 stamp 默认值，或 walkaround Audit 16 与 conform 的关系时
applies_to: [scripts/flightdeck_conform.py, scripts/flightdeck_conform.js, scripts/tests/test_flightdeck_conform.py, skills/conform/SKILL.md, skills/walkaround/SKILL.md, skills/preflight/templates.md, skills/preflight/protocol.md]
last_updated: 2026-06-20
summary: deck 格式严格化器（strict formatter）——脚本趟做形（删非 schema frontmatter、stamp rules 记录式配置、补 cockpit/rules 缺失 section、输出 worklist），AI 趟填义（重塑 cockpit/rules、补必填语义字段）；排除 archive/ 与 references/（IMPORTED_KIND）；无登记、无历史、无 undo
---
