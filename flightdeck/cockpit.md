...
- [2026-06-07-graduate-and-knowledge-freshness-rollout.md](plans/2026-06-07-graduate-and-knowledge-freshness-rollout.md) — 逐文件落地 graduate + when_to_update/stale + status{active,stale,obsolete}：相位1 脚本/lint 机械核心(TDD)→相位2 skill 散文→相位3 docs/incident 对齐→相位4 dogfood 迁移+全验证
...
- **#0 graduate + 知识保鲜（当前活·brainstorm done + 3家AI审核已折进，待 plan）**：新 spec `specs/2026-06-07-graduate-and-knowledge-freshness` 已成文并按 gpt/claude/ds 三审复盘修订。要点：设计稿命中"约束后续开发/大概率复用"判据（从宽）→ 整个 active 期可补打 graduate hint → done 时本体变身常驻 docs（一份、无 archive 双胞胎、可留 rationale 节）；配 `when_to_update`（格式约束+lint 质量闸）→ 自动翻 `stale`，**检测落 exit-ritual + preflight 双仪式**（修掉"持久化空头承诺"洞）。knowledge status = `{active, stale, obsolete}`：**只删 `superseded` 状态值**（留 supersedes 边），`obsolete`=**knowledge 版 done 排水态**（preflight/landing 排进 archive，与排 done 同逻辑）；incident 回归 tripwire 随之搬家——sweep 扩扫 `archive/incidents/`、复发=un-archive 复活。本 deck 实测 0 个 live obsolete/superseded，迁移 trivial。spec 经 2 轮 3 家 AI 审复盘修订定稿。**plan 已出**：`plans/2026-06-07-graduate-and-knowledge-freshness-rollout`（15 task / 4 相位）。相位1 脚本/lint 机械核心全 TDD（status 枚举/钉扣边删 superseded_by/archivable_obsolete/match_signature 扩 archive/when_to_update 形式地板/stale ⚠ 渲染/锚点 helper trailer 式）→ 相位2 skill 散文（templates/protocol/exit-ritual+preflight/new/landing，spec 当内容源）→ 相位3 docs/incident 对齐（model-architecture/spec-lifecycle 走"更新既有 docs"路、本 spec 不 graduate）→ 相位4 dogfood 迁移+全验证。锚点存储选型已定=git `Flightdeck-Sync:` trailer。**✅ plan 已执行完毕（subagent-driven，15 task 全绿）**：相位1 脚本/lint 7 task TDD（commit b77d705→dbafd84）、相位2 skill 散文 4 task（含 2 处控制器内联一致性修：folder-semantics incident 排水 c21b5e3 + exit-ritual 钉扣边 26001d0）、相位3 docs/incident 对齐、相位4 验证。**175 测试绿、deck lint/index --check/verdict 全 clean、迁移扫描 0 个 live obsolete/superseded**。walkaround 仅余 references/claude-mem 导入既有 dangling-ref（非本次）+ AGENTS.md 待 closeout 重生。**下一步：等用户签收 → 翻 spec+plan done + landing 收尾（archive 双件 + Flightdeck-Sync trailer + re-emit AGENTS.md）。`→done` 是用户断言，不自评。**
