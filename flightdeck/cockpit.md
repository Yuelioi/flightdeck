...
- **preflight 入口不读路由头**(2026-06-26,用户带回两个外部参考 deck 实测)→ `knowledge/routing-headers-not-resident-at-entry.md`。AI 跑 preflight 自陈「根本没读订阅开头,连本仓库知识开头也没读」;dogfood 当场复现(本会话开头我只 `ls` 没读头)。根因:旧 step 3 把扫头框成**条件兜底**(「ls 不够才 grep」),`READ WHEN:` 永不进上下文 → 触发器没承载体 → 订阅+知识全成死重。与 [[sibling-workflow]]/[[persist-no-heartbeat]] 同源(散文把 load-bearing 动作写成可选)。
  ...
  **本会话续加(读取/写入两侧规范)**:① 读取侧——hop1 扫完按标准档**显式展示**(`In force` 点名常驻约定 / `On call` 计数+域,触发才拉);铁律「排序是延后非丢弃」防低档知识被永久撇下;附带=可见证明头真扫了。② 写入侧——`SKILL.md` Routing-header 不变式 + `operations.md` 加「头的形状即优先级信号」作者提示(想常驻→proactive READ-WHEN+checklist;坑→reactive+`# ⚠`)。改:`SKILL.md` step3+不变式、`operations.md` ranking 段。
