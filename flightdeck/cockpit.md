...
Updated: 2026-06-26 · 月离 · 读头分层:协议加「标准化分档展示(In force/On call)」+「写头即定优先级」作者提示;+隐私 finding:外部参考项目名别进 commit(已写 knowledge,历史不回溯)。待办:终端 GBK 复制乱码(环境层,已诊断未交付修法)
...
- **preflight 入口不读路由头**(2026-06-26,用户带回两个外部参考 deck 实测)→ `knowledge/routing-headers-not-resident-at-entry.md`。AI 跑 preflight 自陈「根本没读订阅开头,连本仓库知识开头也没读」;dogfood 当场复现(本会话开头我只 `ls` 没读头)。根因:旧 step 3 把扫头框成**条件兜底**(「ls 不够才 grep」),`READ WHEN:` 永不进上下文 → 触发器没承载体 → 订阅+知识全成死重。与 [[sibling-workflow]]/[[persist-no-heartbeat]] 同源(散文把 load-bearing 动作写成可选)。
  **已改(发布面协议,与用户 brainstorm 后定 A=从现有头语义推导、不加字段)**:「先地图后领土」两跳——① hop1 扫头**每次入口必做**(从兜底提为默认);② 给地图按 READ-WHEN 形状**分层**(proactive/checklist→活约束常驻;reactive/`# ⚠` trap→挂索引、症状才拉 body;comments/commits 不分任务常驻);③ hop2 body 仍懒。改:`SKILL.md` step1+3、`operations.md` Derived listing、`README` 中英第2步。**验证**:claude-mem 3-layer / ReMe hop-based 独立推出同模式(见 [[external-memory-borrowings]] #5/#3);跨外部参考 deck A(76 知识/65 reactive trap)+ deck B(59/36 trap/11 checklist)规模成立——deck 越大分层越值钱。**改不了的**:仍靠 AI 记得扫+排序,非物理强制。**已同步 alpha.4 缓存(build c474c8a,未 bump/未 push),待在外部 deck 新会话实测分层手感。**
  **本会话续加(读取/写入两侧规范)**:① 读取侧——hop1 扫完按标准档**显式展示**(`In force` 点名常驻约定 / `On call` 计数+域,触发才拉);铁律「排序是延后非丢弃」防低档知识被永久撇下;附带=可见证明头真扫了。② 写入侧——「头的形状即优先级信号」作者提示,且把**具体提权词表钉死**(不留「你觉得写啥词」的含糊):In force = 无条件/规范措辞「before **any** \<action>」+ SUMMARY 祈使「Always…/Never…」+ checklist 标题;On call = 症状「when \<X> shows」+`# ⚠`;「any/every/always/never」提权,描述式「notes on…」下沉。**词表放进始终加载的 micro-core 不变式**(避免重蹈「只在按需层=永不读」),全表在 `operations.md` Authoring side。改:`SKILL.md` step3+Routing-header 不变式、`operations.md` ranking+authoring 段。
- **外部参考项目名泄露进 committed 面**(2026-06-26)→ `knowledge/reference-project-names-leak-into-commits.md`。给你参考的项目名被我写进 commit message+cockpit+knowledge,repo 是 public-bound 会泄露。规则:committed 面一律匿名化(「外部参考 deck A/B」+ 只留规模/结构等可移植事实,不留名/路径/URL)。**历史不回溯**(用户:之前的算了),只保证此后干净;本回合已把 cockpit/knowledge 里的名换成中性标号。
