...
Updated: 2026-06-23 · 月离 · Stage: brainstorming「AI-native 重设」（本周期锤定 5 条：位置替状态 + 冷存储进母库，边锤边填）— plan 3 仍 staged 待 land
...
- **继续锤 AI-native 重设** — 本周期已锤定 5 条（位置替状态 / 冷存储进母库 / 母库不版本化 / 砍 INDEX / 砍自动 stale，连带 git-undo·多 agent·机械兜底出局）。剩「还要锤」：3.0→新形态一次性迁移 · frontmatter 必留 vs 可松字段表 · 派生目录约定 · 产品化去留 · 极简协议正文。brainstorm 进行中 → `specs/2026-06-23-ai-native-redesign.md`。
