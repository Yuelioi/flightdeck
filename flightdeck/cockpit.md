...
- **继续锤 AI-native 重设** — 本周期已锤定 6 条（位置替状态 / 冷存储进母库 / 母库不版本化 / 砍 INDEX / 砍自动 stale / **frontmatter 全砍→零 schema**，连带 git-undo·多 agent·机械兜底·迁移痛 #2 出局）。产品化方向已锁（doc-first 薄产品）+ 协议正文形态已定（两层：微核心常载 + 深层惰加载）。设计性大叉基本锤完——剩偏「写/执行」：极简协议 authoring（英文发布面）· 3.0→新形态一次性迁移 · 派生目录触发阈值。brainstorm 进行中 → `specs/2026-06-23-ai-native-redesign.md`。
