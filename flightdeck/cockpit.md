...
- [2026-06-19-shared-knowledge-sync-v2.md](specs/2026-06-19-shared-knowledge-sync-v2.md) — 母库解析塌成固定约定 ~/.flightdeck（symlink/junction 逃生口）；synced_from 去别名改 synced 标记；母库加 consumers 注册表（拷贝时剔除）+ 脚本自动维护 + /flightdeck:sync --fanout 真扇出到全下游。
- [2026-06-19-shared-knowledge-sync-v2.md](plans/2026-06-19-shared-knowledge-sync-v2.md) — 落地 v2 spec：母库解析固定 ~/.flightdeck + synced 去别名 + consumers 注册表(register/list/prune)+ /flightdeck:sync --fanout 编排 + 配置面清理 + v1 迁移补注册。
<!-- /AUTO -->
