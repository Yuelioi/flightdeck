...
<!-- DRAFT (sub-plan #4, redesign) — this is what skills/walkaround/SKILL.md becomes
in the new form. Lives in work/ until the cutover; does not touch live skills/. The
3.0 walkaround's ~16 schema/INDEX/sync audits collapse to five, because the new form
has no schema, no INDEX, and no sync subsystem to police. -->
...
3. **Duplicate traps.** Two `# ⚠ <title>` trap files describing the same pitfall (same
   ...
   trap, not spawn a copy (see `protocol.md` § Incidents).
