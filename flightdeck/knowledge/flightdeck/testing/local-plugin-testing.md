...
SUMMARY: How to locally test an unpublished flightdeck build / sync the working tree into Claude and Codex plugin caches.
READ WHEN: before locally testing an unpublished build / syncing the working tree into local plugin caches

---

## When to follow this

When dogfooding an **unpublished** flightdeck build (e.g. a `3.x` branch) before cutting a release. The live install Claude Code / Codex loads is the **plugin cache**, not the repo — so testing means overwriting those caches with the working tree, then restarting the session.

> **Sync only on explicit say-so.** Don't re-mirror after every edit. Make repo edits freely, leave the caches stale, and run the sync once, when the user explicitly says "复制"/"copy". (Why: re-syncing unprompted is noisy and removes the user's control over when test changes go live.)

## Locate the live caches
...
1. Codex home is **`$env:CODEX_HOME`** when set, otherwise `~\.codex`.
2. Cache path: `$CodexHome\plugins\cache\flightdeck-marketplace\flightdeck\<version>\`.
3. The Codex plugin must already be installed once; the local sync script refuses to create a missing plugin cache from scratch.
4. `scripts\sync_codex_cache.ps1` auto-detects the plugin name from `.codex-plugin\plugin.json` and, when `-Version` is omitted, uses the most recently modified installed version directory.

## Sync the working tree into both caches

For local testing, sync **both** live caches in the same pass: Claude Code and Codex. Stamp the repo once first, then mirror that same stamped tree into both destinations.

```powershell
$src = "<flightdeck repo root>"   # the working tree you're testing
$claudeDst = "$env:CLAUDE_CONFIG_DIR\plugins\cache\flightdeck-marketplace\flightdeck\<published-version>"
...
# Claude Code cache.
robocopy $src $claudeDst /MIR /XD .git tmp .vscode .superpowers references __pycache__ .pytest_cache /XF .in_use *.pyc *.pyo
# robocopy exit code 0–7 = success; >=8 = failure.

# /MIR deletes .in_use (it is NOT in $src). Recreate it (see gotcha below):
New-Item -ItemType Directory -Force "$claudeDst\.in_use" | Out-Null
$m = Get-Item "$claudeDst\.in_use" -Force
$m.Attributes = $m.Attributes -bor [System.IO.FileAttributes]::Hidden

# Codex cache. -SkipStamp is important because the stamp was already written above.
.\scripts\sync_codex_cache.ps1 -RepoRoot $src -SkipStamp
```
...
Use the Claude block above by itself only when the test target is explicitly Claude-only. Otherwise keep local dogfood caches in parity and sync both.

### Codex-only sync

Use the helper when the test target is explicitly Codex-only:
...
- `-CodexHome <path>`: override `$env:CODEX_HOME` / `~\.codex`.
- `-Version <version>`: mirror into a specific installed cache version instead of the most recently modified version.
- `-DryRun`: show the robocopy plan without modifying the Codex cache.
- `-SkipStamp`: use only when `uv run scripts\build_stamp.py --write` has already run in the same sync pass.
...
- **Restart the session.** The running session already loaded the *old* skills into context; the synced build only takes effect in a **new** Claude Code / Codex session.
...
- Claude changed files landed: `Get-FileHash $src\skills\preflight\SKILL.md` equals `Get-FileHash $claudeDst\skills\preflight\SKILL.md`.
- Codex changed files landed: `Get-FileHash $src\skills\preflight\SKILL.md` equals `Get-FileHash <codex-dst>\skills\preflight\SKILL.md`.
- Deletions propagated: a file removed in the repo is absent under `$claudeDst` and `<codex-dst>`.
- Claude `.in_use` exists and is a directory: `(Get-Item "$claudeDst\.in_use" -Force).PSIsContainer` → `True`.
- New sessions: the Claude skill list / `/flightdeck:preflight` and the Codex skill list / force-invoke path reflect the new behavior.
